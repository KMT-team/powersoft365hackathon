 ...
 * ui.js
 * 
 * This module handles all modifications to the DOM (Document Object Model).
 * It receives data from the Store and turns it into HTML.
 * 
 * We use Template Literals (backticks) to create HTML strings easily.
 */
...
const elements = {
    ...
    editModalInputs: {
        ...
        variantsContainer: document.getElementById('edit-variants-container')
    }
};

/**
 * Pre-fills the Edit Modal with product data.
 * Handles Role-based UI states.
 */
export function populateEditModal(product, role = 'admin') {
    ...
    // Disable main fields for employees
    ...
    const employeeControls = document.getElementById('employee-controls');

    // 1. Handle Admin-specific controls (Delete, Save, Inputs)
    if (isEmployee) {
    ...
    // 2. Handle Deactivation Controls (Available to ALL roles now)
    employeeControls.classList.remove('hidden');

    // Setup Activate/Deactivate visibility based on product state
    ...
    const deactivationSection = document.getElementById('deactivation-section');
    ...
    elements.editModalInputs.variantsContainer.innerHTML = product.variants.map((v, index) => {
        ...
        // Disable variant inputs for employees
        const inputDisabled = isEmployee ? 'disabled' : '';
        const removeBtnClass = isEmployee ? 'hidden' : 'btn-remove-variant'; // Hide remove button for employees

        return `
        <div class="variant-row grid grid-cols-12 gap-2 items-center text-sm">
            <input type="text" value="${v.color}" placeholder="Color" ${inputDisabled} class="variant-color col-span-3 px-2 py-1.5 border border-gray-300 rounded focus:ring-1 focus:ring-indigo-500">
            <select ${inputDisabled} class="variant-size col-span-2 px-2 py-1.5 border border-gray-300 rounded focus:ring-1 focus:ring-indigo-500">
                ${sizeOptions}
            </select>
            <input type="number" value="${v.stock}" min="0" placeholder="Stock" ${inputDisabled} class="variant-stock col-span-5 px-2 py-1.5 border border-gray-300 rounded focus:ring-1 focus:ring-indigo-500">
    ...
    // Hide/Show Add Variant button for employees (safe check)
...
/**
 * Renders the list of products into the grid.
...
/**
 * INTERNAL HELPER: Creates the HTML for a single product card.
 */
function createProductCard(product) {
    // Generate variants HTML
    // We attach data-id and data-variant-index to the Sell button to know what to update later.
    ...
    return `
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow flex flex-col ${opacityClass} relative">
            ${activeLabel}
            <!-- Card Header -->
            <div class="p-5 border-b border-gray-50 bg-gradient-to-br from-white to-gray-50">
                <div class="flex justify-between items-start">
                    <div>
                        <span class="text-xs font-bold text-indigo-500 uppercase tracking-widest">${product.category}</span>
                        <!-- Added 'product-card-title' for hover effect (CSS) and onclick handler -->
                        <h3 
                            onclick="window.handleEdit(${product.id})"
                            class="text-lg font-bold text-gray-900 mt-1 product-card-title cursor-pointer"
                            title="Click to Edit"
                        >
                            ${product.name}
                        </h3>
